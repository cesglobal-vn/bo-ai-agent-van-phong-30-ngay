...
> Tài liệu này dành cho **học viên**. Đọc từ trên xuống là cài được và dùng được bộ 12 AI Agent văn phòng ngay trong **Claude Code**.
...
| Thành phần | Nội dung |
|---|---|
| **12 AI Agent nền** | Thư mục `01`…`12`, mỗi agent gồm `AGENT.md` (đặc tả đầy đủ) + `SKILL.md` (bản cài Skill). Chi tiết ở **Mục 7**. |
...
**Claude Code** là công cụ dòng lệnh (chạy trong Terminal) để làm việc với Claude ngay trên máy bạn — đọc/ghi file, tự động hóa công việc. Nó hỗ trợ **Skills**: các "kỹ năng" đóng gói sẵn mà Claude tự kích hoạt khi bạn nói đúng từ khóa. Bộ AI Agent này chính là 12 Skill như vậy.
...
### Bước 2 — Chép 12 thư mục agent vào thư mục skills

Mỗi thư mục agent (`01-viet-thu-dien-tu`, `02-tom-tat-tai-lieu`, … `12-workspace-ca-nhan`) đã có sẵn `SKILL.md` — đúng chuẩn Skill của Claude Code. Bạn chỉ cần **chép 12 thư mục này** vào `~/.claude/skills/`.

- **Windows (PowerShell):**
  ```powershell
  # Tạo thư mục skills nếu chưa có
  New-Item -ItemType Directory -Force "$HOME\.claude\skills"
  # Chép các thư mục agent (đứng trong thư mục Bo-AI-Agent vừa giải nén)
  Copy-Item -Recurse -Force ".\0*", ".\1*" "$HOME\.claude\skills\"
  ```
- **Mac/Linux (Terminal):**
  ```bash
  mkdir -p ~/.claude/skills
  cp -R 0* 1* ~/.claude/skills/
  ```

> Hoặc đơn giản: mở File Explorer/Finder, copy 12 thư mục `01…12` rồi dán vào `~/.claude/skills/`.
...
3. Claude Code sẽ tự kích hoạt Skill *Viết thư điện tử* (agent số 01) và bắt đầu **hỏi bạn 4–6 câu về ngữ cảnh** trước khi viết. Đó là dấu hiệu cài đúng.
...
Mỗi ngày mở file `Lo-trinh-30-ngay/Tuan-X/Ngay-XX-.../BAI-HOC.md` — có đủ mục tiêu, bài toán, AGENT.md rút gọn, SKILL.md, diễn tiến 45 phút, bài tập về nhà, tiêu chí đạt.
...
> *Tuần 2 (ngày 08–10: Báo cáo chỉ số · Gợi ý biểu đồ · Tóm tắt cho quản lý) là biến thể chuyên sâu của agent 05 và 07 — xem file `BAI-HOC.md` từng ngày.*
...
> *Tuần 3 (ngày 12: Thông báo nội bộ · ngày 15: Kho hỏi đáp) là biến thể của agent 01 và 02 — xem `BAI-HOC.md`.*
